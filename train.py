...
class NodeClsTrainer:
    ...
    def train(self, ff):
        ...
        output = model(data)
        if ff:
            plot_TSNE('cora', 'GCNMF', data.test_mask, model.embedding, data.labels)
        loss = F.nll_loss(output[data.train_mask], data.labels[data.train_mask])    # 求解loss损失
        loss.backward()
        optimizer.step()
